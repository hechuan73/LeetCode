...
public class KnuthMorrisPratt {
    ...
    /**
     * Knuth-Morris-Pratt (KMP)算法是单模式串匹配算法。算法主要思想是主串与模式串从前往后匹配，遇到的第一个不能匹配的字符为"坏字符"，已经匹
     * 配上的子串为"好前缀"。每次在好前缀串中查找能够最长的匹配好前缀串后缀子串的前缀子串，并将模式串移动，使得最长的可匹配的前缀子串与后缀子串对
     * 其，然后将坏字符与最长可匹配前缀子串的下一个字符进行比较。算法的思想也是在遇到不能匹配的字符时，可以将模式串向后移动一定长度，而不是移动一
     * 位逐一比较。
     *
